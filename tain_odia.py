...
def train(args, model, optimizer, train_dataloaders, train_datasets_names, lr_scheduler, valid_dataloaders, valid_datasets_names):
    ...
    for epoch in range(epoch_start, epoch_num):
        ...
        for data in metric_logger.log_every(train_dataloaders, 50):
            inputs = data['image']
            labels = data['word_masks'].to(args.device)
            batched_input = []
            if args.hier_det:
...
if __name__ == "__main__":
    ...
    odia_text = {
        "name": "OdiaText",
        "im_dir": "./datasets/odia_text/train",
        "gt_dir": "",   # IMPORTANT (since you use JSON)
        "im_ext": ".jpg",
        "gt_ext": ".jpg",
        "json_dir": "./datasets/odia_text/train.json"
    }
    ...
    odia_text_val = {
        "name": "OdiaText-val",
        "im_dir": "./datasets/odia_text/valid",
        "gt_dir": "",
        "im_ext": ".jpg",
        "gt_ext": ".jpg",
        "json_dir": "./datasets/odia_text/valid.json"
    }
    
    val_dataset_map = {
        'odia_text': odia_text_val,
