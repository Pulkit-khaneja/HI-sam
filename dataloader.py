...
def create_dataloaders(
    ...
    # Detect whether distributed training is active.
    # DistributedSampler requires dist.init_process_group() to have been called;
    # fall back to standard samplers when running on a single process/GPU.
    use_distributed = (
    ...
    if training:
        ...
        if use_distributed:
            ...
            # Single-GPU / CPU: use a plain shuffling sampler
            dataloader = DataLoader(
                gos_dataset,
...
class OnlineDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if 'TextSeg' in self.dataset_name:
            ...
            gt = (gt_ori > 127).astype(np.uint8) * 255  # for TotalText, HierText, Custom: 0 or 255
        if len(gt.shape) > 2:
            gt = gt[:, :, 0]
        if len(im.shape) < 3:
            im = im[:, :, np.newaxis]
        ...
        if self.hier_det:
            ...
            if 'HierText' in self.dataset_name:
                if line_num > 10:
                    ...
                    masks = np.array(masks).transpose((1, 2, 0))
                    sample['word_masks'] = masks
                    line2para_index = anns['line2paragraph_index']
