...
def get_args_parser():
    ...
    parser.add_argument("--train_datasets", type=str, nargs='+', default=['custom_train'])
    parser.add_argument("--val_datasets", type=str, nargs='+', default=['custom_test'])
    ...
    # --- Custom dataset paths (override via CLI) ---
    parser.add_argument('--custom_train_im_dir', type=str, default='./datasets/custom/images/train',
                        help='Path to custom training images directory')
    parser.add_argument('--custom_train_gt_dir', type=str, default='./datasets/custom/masks/train',
                        help='Path to custom training ground-truth masks directory')
    parser.add_argument('--custom_test_im_dir', type=str, default='./datasets/custom/images/test',
                        help='Path to custom test images directory')
    parser.add_argument('--custom_test_gt_dir', type=str, default='./datasets/custom/masks/test',
                        help='Path to custom test ground-truth masks directory')
    parser.add_argument('--custom_im_ext', type=str, default='.jpg',
                        help='Image file extension for custom dataset (e.g. .jpg, .png)')
    parser.add_argument('--custom_gt_ext', type=str, default='.png',
                        help='Mask file extension for custom dataset (e.g. .png, .jpg)')
    # Optional: JSON annotation for hierarchical detection on custom data
    parser.add_argument('--custom_train_json_dir', type=str, default=None,
                        help='(Optional) Path to JSON annotation file for custom train set (hier_det mode)')
    parser.add_argument('--custom_test_json_dir', type=str, default=None,
                        help='(Optional) Path to JSON annotation file for custom test set (hier_det mode)')

    return parser.parse_args()


def build_dataset_maps(args):
    """
    Builds train and val dataset maps dynamically, including the custom dataset
    whose paths are injected from CLI arguments.
    """

    # ------------------------------------------------------------------ #
    #  Custom dataset  (paths come from --custom_* CLI args)              #
    # ------------------------------------------------------------------ #
    custom_train = {
        "name": "Custom-train",
        "im_dir":  args.custom_train_im_dir,
        "gt_dir":  args.custom_train_gt_dir,
        "im_ext":  args.custom_im_ext,
        "gt_ext":  args.custom_gt_ext,
    }
    if args.custom_train_json_dir is not None:
        custom_train["json_dir"] = args.custom_train_json_dir

    custom_test = {
        "name": "Custom-test",
        "im_dir":  args.custom_test_im_dir,
        "gt_dir":  args.custom_test_gt_dir,
        "im_ext":  args.custom_im_ext,
        "gt_ext":  args.custom_gt_ext,
    }
    if args.custom_test_json_dir is not None:
        custom_test["json_dir"] = args.custom_test_json_dir

    # ------------------------------------------------------------------ #
    #  Built-in datasets (kept for backward-compatibility)                #
    # ------------------------------------------------------------------ #
    totaltext_train = {
    ...
    train_dataset_map = {
        'custom_train':       custom_train,         # <-- your custom dataset
    ...
    val_dataset_map = {
        'custom_test':    custom_test,              # <-- your custom dataset
    ...
    return train_dataset_map, val_dataset_map
...
def main(train_datasets, valid_datasets, args):
    ...
    ### --- Step 2: DistributedDataParallel (only when multi-GPU) ---
    ...
    if args.world_size > 1:
        ...
        # Single-GPU / CPU: skip DDP, expose model directly
...
def train(args, model, optimizer, train_dataloaders, train_datasets_names, lr_scheduler, valid_dataloaders, valid_datasets_names):
    ...
    # Works whether DDP is active (model.module) or single-GPU (model directly)
    _model_core = model.module if hasattr(model, 'module') else model
    ...
    for epoch in range(epoch_start, epoch_num):
        ...
        sampler = getattr(getattr(train_dataloaders, 'batch_sampler', None), 'sampler', None)
        if sampler is not None and hasattr(sampler, 'set_epoch'):
            sampler.set_epoch(epoch)

        for data in metric_logger.log_every(train_dataloaders, 50):
            inputs, labels = data['image'], data['label'].to(model.device)
            batched_input = []
            if args.hier_det:
                ...
                if args.hier_det:
                    ...
                    if word_masks is not None:
                        ...
                        loss_mse_line = loss_hi_iou_mse(
                            hi_iou_output[:, 1:2], hi_masks_logits[:, 1:2, :, :], _model_core.mask_threshold, line_masks
                        )
                        ...
                        loss_mse_para = loss_hi_iou_mse(
                            hi_iou_output[:, 2:3], hi_masks_logits[:, 2:3, :, :], _model_core.mask_threshold, para_masks
                        )
        ...
        if (epoch - epoch_start) % args.valid_period == 0 or (epoch + 1) == epoch_num:
            if args.hier_det:
                _model_core.hier_det = False
            test_stats = evaluate(args, model, valid_dataloaders, valid_datasets_names)
            if args.hier_det:
                _model_core.hier_det = True
            if misc.is_main_process():
                for ds_idx, (ds_name, ds_results) in enumerate(test_stats.items()):
                    iou_result = ds_results.get('001-text-IOU', None)
                    iou_result_hs = ds_results.get('001-text-IOU_hr', None)
                    # For custom datasets the evaluator is skipped and IOU is not computed;
                    # save a checkpoint every validation period instead of gating on IOU.
                    if iou_result is None or iou_result_hs is None:
                        checkpoint = {
                        ...
                        torch.save(checkpoint, os.path.join(args.output, ds_name + f"_epoch{epoch}.pth"))
                        print(f"[Checkpoint] Saved {ds_name}_epoch{epoch}.pth (no IOU metric for custom dataset)")
                    else:
    ...
    if misc.is_main_process():
        ...
        torch.save(_model_core.state_dict(), args.output + model_name)
...
def evaluate(args, model, valid_dataloaders, valid_datasets_names):
    ...
    for k in range(len(valid_dataloaders)):
        ...
        try:
            ...
            print('============================')
            print(f"[WARNING] Evaluator does not support dataset '{valid_dataset_name}' — skipping evaluation.")
            print("To enable evaluation, implement a custom Evaluator or use a supported dataset.")
            print('============================')
            # Return a dummy result so training can continue and checkpoints can still be saved
            test_stats.update({valid_dataset_name: {'001-text-IOU': 0.0, '001-text-IOU_hr': 0.0}})
            continue
...
if __name__ == "__main__":
    ...
    # Build dataset maps dynamically (custom paths injected from CLI args)
    train_dataset_map, val_dataset_map = build_dataset_maps(args)

    train_datasets = []
    val_datasets = []

    for ds_name in args.train_datasets:
        if ds_name not in train_dataset_map:
            raise ValueError(f"Unknown train dataset: '{ds_name}'. Available: {list(train_dataset_map.keys())}")
        train_datasets.append(train_dataset_map[ds_name])

    for ds_name in args.val_datasets:
        if ds_name not in val_dataset_map:
            raise ValueError(f"Unknown val dataset: '{ds_name}'. Available: {list(val_dataset_map.keys())}")
        val_datasets.append(val_dataset_map[ds_name])

    # Print resolved dataset paths for easy debugging
    print("=== Train datasets ===")
    for ds in train_datasets:
        print(f"  {ds['name']}: images={ds['im_dir']}  masks={ds['gt_dir']}")
    print("=== Val datasets ===")
